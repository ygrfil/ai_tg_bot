...
def get_llm(selected_model: str, stream_handler: Any, user_id: int) -> BaseChatModel:
    llm_config: Dict[str, tuple] = {
        ...
        "gemini": (ChatOpenAI, {"api_key": ENV.get("GOOGLE_API_KEY"), "model": MODEL_CONFIG.get("gemini_model"), "temperature": float(MODEL_CONFIG.get("gemini_temperature")), "max_tokens": 1024}),
    }
